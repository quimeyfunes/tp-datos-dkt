...
: Salvable(ruta,sobreEscribir,tamElemento){
	...
	//levanta la tabla de disco y la cargar en memoria...
		void* buff = leerBloque(0);
		tamTabla = Serializador::desSerializarInt(buff);
	}
}

int HashTable::getElement(const ContextoHash& key){
	//hasheo la contexto...
	int pos = key.hashear(tamTabla);
	//obtengo el bloque y devuelvo el valor...
	void* buff = leerBloque(pos+1);
	int retorno =Serializador::desSerializarInt(buff);
	delete[] (char*)buff;
	return retorno;
}
...
int HashTable::getElement(int pos){
	//obtengo el bloque y devuelvo el valor...
	void* buff = leerBloque(pos+1);
	return Serializador::desSerializarInt(buff);
}
void HashTable::setElement(int elem, const ContextoHash& key){
	if (tamTabla == 0){
		tamTabla++;
		guardarTamTabla();
	}
...
int HashTable::nodoNuevo(int tamDispersion, const ContextoHash& key){
...
int HashTable::nodoLibre(int tamDispersion, const ContextoHash& key){
	int pos = key.hashear(tamTabla);
...
void HashTable::duplicarTabla(){
	...
	for (int i=0; i<fin; i++){
		int pos = i+1;
		void* buff = leerBloque(pos);
		//guardo el bloque al final del archivo...
		escribirBloque(buff, sizeof(int), tamTabla+pos);
	}
...
void HashTable::guardarTamTabla(){
	try {
		void* buff[sizeof(int)];
		Serializador::serializarInt(tamTabla, buff);
		escribirBloque(buff, sizeof(int), 0);
...
void HashTable::guardarElem(int elem, int pos){
	void* buff[sizeof(int)];
	Serializador::serializarInt(elem, buff);
	escribirBloque(buff, sizeof(int), pos+1);
}

ostream& operator << (ostream& o, HashTable& tabla){
	o << "Tabla:\n\t";
	for (int i=0; i<tabla.tamTabla; i++){
		void* buff = tabla.leerBloque(i+1);
		o << Serializador::desSerializarInt(buff) << " ";
		delete[] (char*)buff;
		//string buf;
		//TODO CAMBIAR A CHAR*
